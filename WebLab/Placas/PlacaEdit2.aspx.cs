...
namespace WebLab.Placas {
    public partial class PlacaEdit2 : System.Web.UI.Page {
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["idUsuario"] != null)
                {
                    ...
                    if (Request["placa"] != null)
                    {
                        ...
                        if (Request["placa"].ToString() == "Alplex7V")
                        { lblEquipo.Text = "Alplex7V"; pnlPromega.Visible = false; pnlAlplex.Visible = true; pnlPromega2.Visible = false; }
                    }
        ...
        private void CargarListas()
        {
            ...
            string m_ssql = @"SELECT   distinct idusuario, apellido  as nombre 
                FROM sys_usuario (nolock) WHERE firmavalidacion like '%Bio%' and idEfector=" + oC.IdEfector.IdEfector.ToString()+ " and activo=1 order by nombre ";
        ...
        private void Guardar(Placa oRegistro, string estado)
        {
            ...
            string m_strSQL = @"select distinct idItem from LAB_DetalleTipoPlaca where [idTipoPlaca] =2";//Caro
            if (oRegistro.Equipo == "Alplex7V")
            {
                m_strSQL = @"select distinct idItem from [LAB_DetalleTipoPlaca] where [idTipoPlaca] =3";//Caro
            }
            ...
            for (int i = 0; i < Ds.Tables[0].Rows.Count; i++)
            {
                ...
                if (oRegistro.Equipo.Substring(0, 6)== "Alplex")//tmb alplex8V
                {
                  //  oRegistro.BorrarDetalle();
