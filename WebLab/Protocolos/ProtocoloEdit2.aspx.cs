    ...
    public partial class ProtocoloEdit2 : System.Web.UI.Page {
        ...
        public string GetPuco(int numeroDocumento) {
            ...
            try {
                ...
                if (nombre == "")
                    nombre = "Sin obra social"; // sin obra social
                else
            ...
            if (!Page.IsPostBack) {
                ...
                if (Session["idUsuario"] != null) {
                    ...
                    if (Request["Operacion"].ToString() == "Modifica") {
                        ...
                        if (oC.ValorDefectoFechaOrden == 0)
                            txtFechaOrden.Value = "";
                        else
                            txtFechaOrden.Value = DateTime.Now.ToShortDateString();

                        if (oC.ValorDefectoFechaTomaMuestra == 0)
                            txtFechaTomaMuestra.Value = "";
                        else
                        ...
                        if (Request["idServicio"].ToString() == "6") {
                            ...
                            if (Request["Operacion"].ToString() == "AltaPeticion")
                            {
                                ...
                                if (oRegistro.IdOrigen.IdOrigen == 3)
                                    ddlPrioridad.SelectedValue = "2"; // si es de guardia es urgente
                                if (oRegistro.IdTipoServicio.IdTipoServicio == 3)
                                    ddlPrioridad.SelectedValue = "1"; // si es de microbiologia NO es urgente
        ...
        private void MuestraDatos() {
            ...
            if (oRegistro != null) {
                ...
                if (oRegistro.tieneAdjuntoProtocolo()) { spanadjunto.Visible = true; } else { spanadjunto.Visible = false; }
                ...
                if (oRegistro.IdCasoSISA > 0) { lblNroSISA.Text = "Nro. SISA: " + oRegistro.IdCasoSISA.ToString(); lblNroSISA.Visible = true; }
                txtEspecialista.Text = oRegistro.MatriculaEspecialista;
                string espe = oRegistro.Especialista;
                string matricula = oRegistro.MatriculaEspecialista + '#' + oRegistro.Especialista;
                //      MostrarMedico();
                    ...
                    if (oRegistro.IdCasoSISA > 0)
                        ...
                        if (oC.NotificarSISA) {
                                                                       ...
                                                                       //                                                                //   if ((oRegistro.IdCaracter == 1) || (oRegistro.IdCaracter == 8)) //sospechoso o detectar
                            {
                                ...
                                if (idItem != "") { btnNotificarSISA.Visible = true; }
                            }
                        }
                    }
...
where pd.idProtocolo=" + oRegistro.IdProtocolo.ToString();
        ...
        protected void btnGuardar_Click(object sender, EventArgs e) {
            if (Page.IsValid) { ///Verifica si se trata de un alta o modificacion de protocolo               
                Business.Data.Laboratorio.Protocolo oRegistro = new Business.Data.Laboratorio.Protocolo();
                if (Request["Operacion"].ToString() == "Modifica")
                    oRegistro = (Business.Data.Laboratorio.Protocolo) oRegistro.Get(typeof(Business.Data.Laboratorio.Protocolo), int.Parse(Request["idProtocolo"].ToString()));
                ...
                if (guardo) {    ////if (oRegistro.IdTipoServicio==6) Response.Redirect ("ProtocoloMensaje.aspx")
                    ...
                    if (Request["Operacion"].ToString() == "Modifica") {
                        if (Request["DesdeUrgencia"] != null)
                            ...
                            switch (Request["Desde"].ToString()) {
                                case "Default":
                                    Response.Redirect("Default2.aspx?idServicio=" + Session["idServicio"].ToString(), false);
                                    break;
                                case "ProtocoloList":
                                ...
                                case "Control":
                                    ...
                                    break;
                                case "Urgencia":
                                    Response.Redirect("../Urgencia/UrgenciaList.aspx", false);
                                    break;
                                ...
                                if (Session["idUrgencia"] != null) {
                                    switch (Session["idUrgencia"].ToString()) {
                                        ...
                                        case "2": // va a la carga de otro protocolo con urgencia
                                            Response.Redirect("Default2.aspx?idServicio=" + Session["idServicio"].ToString() + "&idUrgencia=2", false);
                                            break;
                                        case "0":
        ...
        private void ActualizaEstadoLote(int idLote, Protocolo oRegistro) {
            try {
                if (idLote != 0) {
     ...
     VALUES   ( " + oProt.IdProtocolo.ToString() + "," + oUser.IdEfector.IdEfector.ToString()+"," + s_area + ",0,'" + impresora + "' , getdate()    )";
        ...
        private bool Guardar(Business.Data.Laboratorio.Protocolo oRegistro) {
            ...
            if (IsTokenValid()) {
                ...
                {
                    if (chkSinFIS.Checked)
                        grabarincidenciafis = true;
                    else
                        oRegistro.FechaInicioSintomas = DateTime.Parse(txtFechaFIS.Value);
                ...
                {
                    if (chkSinFUC.Checked)
                        grabarincidenciafuc = true;
                    else
                        oRegistro.FechaUltimoContacto = DateTime.Parse(txtFechaFUC.Value);
                    //GRABAR fUC
                }
                ...
                if (Request["Operacion"].ToString() != "Modifica")
                    if (Request["Operacion"].ToString() != "AltaTurno") {
                        ...
                        if (ddlMuestra.SelectedValue != "0")
                            s_valores += "@ddlMuestra:" + ddlMuestra.SelectedValue;
        ...
        private void GuardarDetalle(Business.Data.Laboratorio.Protocolo oRegistro) {
            ...
            for (int i = 0; i < tabla.Length - 1; i++) {
                ...
                if (codigo != "") {
                    ...
                    if (listadetalle.Count == 0) { //// si no está lo agrego --- si ya está no hago nada
                ...
                if (Request["DesdeUrgencia"] != null)
                    ...
                    switch (Request["Desde"].ToString()) {
                        ...
                        case "Urgencia":
                            ...
                            break;
                        case "Derivacion":
                            Response.Redirect("Derivacion.aspx?idServicio=" + Session["idServicio"].ToString(), false);
                            break;
                    }
            ...
            if (ddlEfector.SelectedValue != oC.IdEfector.IdEfector.ToString()) {
                ...
                try {
                    ...
                    if (oEfectorExterno.IdTipoEfector == 2) // es privado
                    {
                        ...
                        lblAlertaObraSocial.Visible = false;

                    switch (oEfectorExterno.IdZona.IdZona) {
                        case 1: // subse
                            ddlOrigen.SelectedValue = "5";
                            break;
                        case 2: // zona I
                            ddlOrigen.SelectedValue = "6";
                            break;
                        case 3: // zona II
                            ddlOrigen.SelectedValue = "8";
                            break; //7
                        case 5: // zona III
                            ddlOrigen.SelectedValue = "9";
                            break;
                        case 7: // zona IV
                            ddlOrigen.SelectedValue = "10";
                            break;
                        case 8: // zona v
                            ddlOrigen.SelectedValue = "11";
                            break;
                        case 9: // zona metro
                            ddlOrigen.SelectedValue = "5";
                            break;
                        case 10: // rio negro
                            ddlOrigen.SelectedValue = "12";
                            break;
        ...
        private bool VerificaObligatoriedadFIS() {
            ...
            foreach (string item in arrfis) {
                if (item == ddlCaracter.SelectedValue) { obligafis = true; break; }
            }
            return obligafis;
        ...
        private bool VerificarAnalisisContenidos() {
            ...
            if ((devolver) && (listaCodigo != "")) { devolver = VerificarAnalisisComplejosContenidos(listaCodigo); }

            return devolver;

        }

        private bool VerificarAnalisisComplejosContenidos(string listaCodigo) { ///Este es un segundo nivel de validacion en donde los analisis contenidos no estan directamente sino en diagramas
            bool devolver = true;
            ...
            if (Session["ListaProtocolo"] != null) {
                ...
                for (int i = 0; i < lista.Length; i++) {
                    if (ProtocoloActual == int.Parse(lista[i].ToString())) {
                        if (avance == 1) {
                            if (i < lista.Length - 1) {
                                ProtocoloNuevo = int.Parse(lista[i + 1].ToString());
                                break;
                            }
                        } else {
                            if (avance == 0) {
                                if (i < lista.Length - 1) {
                                    ProtocoloNuevo = int.Parse(lista[i].ToString());
                                    break;
                                }
                            } else
                            if (i > 0) {
                ...
                if (matricula == "0") {
                    ...
                    if (s != "0") {
                        ...
                        if (pro.Count > 0) {
                            ...
                            if (pro.Count > 1)
                                if (Request["idProtocolo"] == null) { ddlEspecialista.Items.Insert(0, new ListItem("--Seleccione--", "0")); }
...
@" and C.idCaracter =" + oRegistro.IdCaracter.ToString();
            ...
            try {
                ...
                if (messge.IsSuccessStatusCode) {
                    ...
                    if (respuesta_d.id_caso != "") { //  devolver el idcaso para guardar en la base de datos
        ...
        private void GenerarResultadoSISA(DetalleProtocolo oDetalle, string idPruebaSISA, string idTipoPruebaSISA, string idResultadoSISA, string idEventoSISA) {
            ...
            try {
                ...
                if (id_resultado_a_informar != 0) {
                    ...
                    resultado newresultado = new resultado { // resultado de dni: 31935346
                        derivada = false,
