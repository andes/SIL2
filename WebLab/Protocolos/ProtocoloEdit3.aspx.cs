...
{
    public partial class ProtocoloEdit3 : System.Web.UI.Page
    {
        ...
        private void CargarDeterminacionesTurno()
        {
         ...
         //   ddlEspecialista.Items.Insert(0, new ListItem(oTurno.Especialista, oTurno.MatriculaEspecialista));
...
//            ddlSectorServicio.Items.Insert(0, new ListItem("Seleccione", "0"));
        ...
        private void CargarListas()
        {
            ...
            oConfiguracion = (ConfiguracionCodigoBarra)oConfiguracion.Get(typeof(ConfiguracionCodigoBarra), "IdTipoServicio", oServicio);  //Multiefector: filtrar por efector
            ...
            else
            {
                if (Request["Operacion"].ToString() != "Alta")
                {
                    ...
                    if (oConfiguracion.Habilitado)
                    {
                        ...
                        m_ssql = "select idArea, nombre from Lab_Area WHERE imprimeCodigoBarra=1 and idTipoServicio=" + oServicio.IdTipoServicio.ToString() + "  and baja=0 order by nombre";
            ...
            m_ssql = "SELECT idImpresora, nombre FROM LAB_Impresora ";  //MultiEfector
            ...
            oUtil.CargarCombo(ddlImpresora2, m_ssql, "nombre", "nombre");
            ...
            if ((Session["idServicio"].ToString() == "3") || (Session["idServicio"].ToString() == "6"))//microbiologia o forense microbiologia
            {
                ...
                m_ssql = @"SELECT idMuestra, nombre + ' - ' + codigo as nombre FROM LAB_Muestra  M
                            where (tipo=0 or tipo=1)";
                if (Request["Operacion"].ToString() != "Modifica")  //alta
                    m_ssql += " and baja=0 and exists (select 1 from lab_muestraEfector E where M.idMuestra = E.idmuestra and E.idefector = " + oUser.IdEfector.IdEfector.ToString()+")"; //Multiefector";
            ...
            m_ssql = "SELECT idEfector, nombre FROM sys_Efector  E ";

            //if (Request["Operacion"].ToString() != "Modifica")  //alta
                m_ssql += " where exists (select 1 from LAB_EfectorRelacionado R where E.idEfector = R.idEfectorRel and R.idefector = " + oUser.IdEfector.IdEfector.ToString() +
           ...
           m_ssql += " order by nombre ";
            ...
            m_ssql = "SELECT I.idItem as idItem, I.nombre + ' - ' + I.codigo as nombre " +
                  " FROM Lab_item I  " +
                  " inner join lab_itemEfector IE on IE.idItem= I.iditem and ie.idefector="+ oC.IdEfector.IdEfector.ToString()+
                  " INNER JOIN Lab_area A ON A.idArea= I.idArea " +
        ...
        private void CargarItems()
        {
            ...
            string m_ssql = "SELECT I.idItem as idItem, I.codigo as codigo, I.nombre as nombre, I.nombre + ' - ' + I.codigo as nombreLargo, " +
                           " IE.disponible " +
                            " FROM Lab_item I  " +
                            " inner join lab_itemEfector IE on I.idItem= IE.idItem and Ie.idefector=" + oC.IdEfector.IdEfector.ToString() + //MultiEfector 
                            " INNER JOIN Lab_area A ON A.idArea= I.idArea " +
            ...
            m_ssql = "SELECT idRutina, nombre FROM Lab_Rutina where baja=0 and idTipoServicio= " + Session["idServicio"].ToString() + " order by nombre ";//Ver MultiEfector 
...
where P.numero=" + Request["numeroProtocolo"].ToString() + @" and idsubItem in (" + Request["analisis"].ToString().Replace("|",",") + ")";
        ...
        private void ImprimirCodigoBarrasAreas(Protocolo oProt, string s_listaAreas)
        {
            ...
            for (int i = 0; i <= tabla.Length - 1; i++)
            {
                ...
                string query = @"
INSERT INTO [dbo].[LAB_ProtocoloEtiqueta]
           ([idProtocolo]
           ,[idArea]
           ,[idItem]
           ,[impresora]
,fechaRegistro
     ...
     VALUES
           ( " + oProt.IdProtocolo.ToString() + "," + s_area + ",0,'" + ddlImpresora2.SelectedItem.Text + "' , getdate()    )";
                SqlCommand cmd = new SqlCommand(query, conn);
        ...
        private void Guardar(Business.Data.Laboratorio.Protocolo oRegistro)
        {
            if (IsTokenValid())
            {
                ...
                if (Request["Operacion"].ToString() == "AltaTurno") oRegistro.ImprimirCodigoBarras(ddlImpresoraEtiqueta.SelectedItem.Text, int.Parse(Session["idUsuario"].ToString()));
...
@" and C.idCaracter =" + oRegistro.IdCaracter.ToString();
...

        {
            ...
            try
            {
                ...
                if (body != "")
                {
                    ResultadoxNro.EventoMuestra respuesta_d = jsonSerializer.Deserialize<ResultadoxNro.EventoMuestra>(body);
