...
namespace WebLab
{
    public partial class SeguimientoResultados : System.Web.UI.Page
    {
...
where iditem =" + oItem.IdItem.ToString()+ @" order by resultadocar";
        ...
        private DataTable MostrarDatos( Item oItem )
        {
            ...
            string m_strSQLCondicion = " and IR.idEfector = " + oUser.IdEfector.IdEfector.ToString();
...
case when  Dp.idUsuarioValida>0 then upper(dp.resultadoCar)  else 'PENDIENTE' end   AS 'CoVid-19' ,
...
where DP.idsubitem =" + oItem.IdItem.ToString()  + m_strSQLCondicion +@"
and  ir.baja=0  ";
...
IR.Especialista AS [Solicitante] ,
dP.fechavalida as [F. Resultado],
case when  Dp.idUsuarioPreValida>0 then upper(dp.resultadoCar)  else 'PENDIENTE' end   AS 'CoVid-19' ,
upper(ltrim(dp.observaciones + ' '+ ir.observacionesResultados)) as Observaciones
...
where  
 DP.idsubitem= " + oItem.IdItem.ToString() + m_strSQLCondicion + @"
and  ir.baja=0  ";
